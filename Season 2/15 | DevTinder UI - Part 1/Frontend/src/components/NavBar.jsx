...
const NavBar = () => {
  ...
  return (
    <nav className="bg-bgSecondary w-full h-auto fixed">
      <div className="container mx-auto flex h-full items-center justify-between px-4">
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              className="border-border w-32 2xs:w-48 rounded-xl border-2 p-1 pl-10 text-black"
              placeholder="Search"
            />
          </div>
        </div>
        ...
        <div
          className={`absolute right-0 top-16 h-[calc(100vh-4rem)] w-0 overflow-hidden rounded-sm transition-all duration-300 ease-in-out md:relative md:right-auto md:top-auto md:flex md:h-auto md:w-auto md:items-center md:overflow-visible ${
            showNavbar ? "bg-bgSecondary mt-8 flex w-64 items-start justify-center pt-10" : ""
          }`}
        >
          <ul className="emd:space-x-12 flex flex-col space-x-2 font-medium md:flex-row">
              ...
              return (
                <li key={link}>
                  <NavLink
                    to={"/" + link}
                    className={({ isActive }) =>
                      `hover:text-hover relative flex flex-col items-center justify-center px-4 py-2 font-semibold ${
                        isActive
                          ? "text-hover after:bg-hover after:h-0.5 after:w-full after:content-['']"
                          : ""
                      }`
                    }
                  >
            ...
            <li
              className="text-hover hover:text-hover relative flex flex-col items-center justify-center"
            >
...
const SliderToggle = ({ theme, toggleTheme }) => {
  return (
    <div className="relative flex w-fit items-center rounded-full border-border border-[1.5px]">
      <button
